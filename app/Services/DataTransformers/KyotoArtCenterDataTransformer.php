...
class KyotoArtCenterDataTransformer implements DataTransformerInterface
{
    public function transform(array $eventData): ?array
    {
        Log::info('Dispatching job for Kyoto Art Center event data', ['event_data' => $eventData]);
    ...
    {
        Log::info('Starting transformation process for Kyoto Art Center event data', ['event_data' => $eventData]);

        // Ensure the organization field is set to "Kyoto Art Center"
        $eventData['organization'] = 'Kyoto Art Center';
        ...
        if (empty($eventData['event_link'])) {
            Log::warning('event_link is missing from Kyoto Art Center event data.', ['event_data' => $eventData]);
        }
    ...
    {
        Log::info('Processing Kyoto Art Center event data for saving', ['event_data' => $eventData]);
    ...
    {
        try {
            Log::info('Updating existing Kyoto Art Center event', ['event_id' => $eventId]);
    ...
    public function constructPrompt(array $eventData): string
    {
        ...
        $prompt = <<<EOT
Given the following JSON data extracted from Kyoto Art Center's event page, parse and transform it into the specified JSON format. Return only the extracted data in **valid JSON format**. Do not include any comments, explanations, or additional text outside the JSON structure.
...
EOT;
        ...
        Log::info('Constructed prompt for Kyoto Art Center', ['prompt' => $prompt]);
    ...
    {
        try {
            Log::info('Creating or updating Kyoto Art Center event', ['event_data' => $eventData]);
