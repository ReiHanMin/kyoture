...
const parsePrices = (rawPriceText) => {
  const priceRegex = /(?:￥|¥)?([\d,]+)(?:\s*\/\s*[\w\s]+)/g;
  const matches = [...rawPriceText.matchAll(priceRegex)];
  const priceTiers = matches.map(match => ({
  ...
  return priceTiers.length ? priceTiers : [{ category: 'General', amount: 0, currency: 'JPY', description: 'No prices found' }];
};
...
const scrapeKyotoConcertHall = async () => {
  ...
  try {
    ...
    for (const eventElement of eventElements) {
      ...
      const priceTiers = parsePrices(eventDetails.price);
...
const parseModalContent = async (modalHTML) => {
  try {
    ...
    return { title, date, venue, program, price, releaseDate };
  } catch (error) {
    console.error('Error parsing modal content:', error);
    return { title: 'No title', date: 'No date', venue: 'No venue', program: 'No program', price: 'No price', releaseDate: 'No release date' };
  }
};
